...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Quiz: {{ $quiz->judul }}</h1>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 p-6 mb-6">
            <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Informasi Quiz</h2>
            <form action="{{ route('pengajar.quiz.update', $quiz->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" rows="3"
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">{{ old('deskripsi', $quiz->deskripsi) }}</textarea>
                </div>

                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Durasi (menit)</label>
                ...
                <div class="mb-4">
                    <div class="flex items-center">
                        ...
                        <label for="is_published" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                            Publish Quiz
                        </label>
                    </div>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 p-6">
            <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Daftar Soal</h2>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 p-6 sticky top-6">
            <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Tambah Soal</h2>
            @if($errors->has('pilihan'))
                <div class="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                    <p class="text-sm text-red-800 dark:text-red-200">{{ $errors->first('pilihan') }}</p>
                </div>
            @endif
            <form action="{{ route('pengajar.quiz.questions.store', $quiz->id) }}" method="POST" id="questionForm">
                ...
                <div class="mb-4" id="pilihanContainer">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pilihan Jawaban</label>
                    <div class="space-y-2">
                        <input type="text" name="pilihan[A]" id="pilihanA" placeholder="A. ..." required
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white pilihan-input">
                    ...
                    <p class="mt-2 text-xs text-red-600 dark:text-red-400 hidden" id="errorMessage">Semua pilihan jawaban harus berbeda!</p>
                </div>
                ...
                <div class="mb-4">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Skor</label>
...
function validatePilihan() {
    ...
    if (duplicates.length > 0) {
        ...
        duplicates.forEach(index => {
            document.getElementById(`pilihan${labels[index]}`).classList.add('border-red-500', 'dark:border-red-500');
        });
        return false;
    } else {
