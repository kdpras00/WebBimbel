...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-white">Tambah Materi</h1>
</div>
    ...
    <form action="{{ route('pengajar.materi.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4" id="textInput">
            ...
            <textarea name="konten" rows="10"
                      class="bg-white border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('konten') }}</textarea>
        </div>
...
if (tipeSelect) {
    tipeSelect.addEventListener('change', function() {
        ...
        if (tipe === 'teks') {
            ...
            fileLabel.innerHTML = 'File PDF <span class="text-red-500">*</span>';
        } else if (tipe === 'video') {
            fileInput.style.display = 'block';
            textInput.style.display = 'none';
            fileInputField.setAttribute('accept', '.mp4,.avi,.mov');
            fileInputField.setAttribute('required', 'required');
            fileLabel.innerHTML = 'File Video <span class="text-red-500">*</span>';
        }
    });
}
