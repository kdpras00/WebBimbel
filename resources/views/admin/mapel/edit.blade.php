        ...
        <div class="mb-4">
            ...
            <select id="namaSelect" class="bg-white border border-gray-300 text-black text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 text-black mb-2">
                ...
                @php
                    ...
                    $currentNama = old('nama', $mapel->nama);
                    $isOther = $currentNama && !in_array($currentNama, $commonSubjects);
                @endphp
                @foreach($commonSubjects as $subject)
                    <option value="{{ $subject }}" {{ $currentNama == $subject ? 'selected' : '' }}>{{ $subject }}</option>
                @endforeach
                <option value="Lainnya" {{ $isOther ? 'selected' : '' }}>Lainnya</option>
            </select>
            
            <input type="text" id="namaInput" name="nama" value="{{ $currentNama }}" 
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    select.addEventListener('change', function() {
                        if (this.value === 'Lainnya') {
                            ...
                            input.value = ''; // Clear input when switching to manual
                            input.required = true;
                            input.focus();
                        } else {
                    ...
                    // Ensure input has value if dropdown is not 'Lainnya' on load (for validation errors or initial load)
